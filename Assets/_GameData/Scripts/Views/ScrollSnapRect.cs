...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
	...
	public GameObject spaNextButton;
	public GameObject makeupNextButton;
    ...
    void Update() {
        ...
        if (_lerp) {
            ...
            float decelerate = Mathf.Min(decelerationRate * Time.deltaTime, 1);
    ...
    private void NextScreen() {
		...
		print ("value of current page above"+_currentPage);
		if (GameManager.instance.currentScene == Utils.CURRENT_SCENE_SPA) {
			MaskGameObject.shouldClearRender = true;
			if (_currentPage >= 4 ) {
				spaNextButton.SetActive (true);
			}
		}
		if (_currentPage >=7 && GameManager.instance.currentScene == Utils.CURRENT_SCENE_MAKEUP) {
			makeupNextButton.SetActive (true);
    ...
    private void PreviousScreen() {
		...
		print ("value of current page below"+_currentPage);
    }
...
//			print ("function value"+":  "+difference);
//			if (difference >= 400 && counter < 3) {
//				counter = counter + 1;
